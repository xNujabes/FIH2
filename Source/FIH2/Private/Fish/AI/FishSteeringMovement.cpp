...
void UFishSteeringMovement::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction *ThisTickFunction)
{
    ...
    if (!PendingInput.IsNearlyZero() && PawnOwner)
    {
        ...
        FRotator NewRotation = FMath::RInterpTo(PawnOwner->GetActorRotation(), TargetRotation, DeltaTime, RotationInterpSpeed);
...
{
    if (!PawnOwner) return DesiredDirection;
    ...
    if (!World->LineTraceSingleByChannel(Hit, PawnLoc, PawnLoc + (ForwardVector * AvoidanceDistance), ObstacleChannel, QueryParams))
    {
    ...
    for (const FRotator& Offset : WhiskerOffsets)
    {
        ...
        if (!World->LineTraceSingleByChannel(Hit, PawnLoc, PawnLoc + (WhiskerDir * AvoidanceDistance), ObstacleChannel, QueryParams))
        {
            return WhiskerDir;
        }
    }
