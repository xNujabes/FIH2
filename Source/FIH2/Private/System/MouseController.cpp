...
void AMouseController::OnMouseClick()
{
	...
	if (OutHits.Num() > 0)
	{
		...
		for (FHitResult HitResult : OutHits)
		{
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("SUCCESS: Interface found! Grabbing Bone: %s"), *HitResult.BoneName.ToString());
				...
				if (RootComp)
				{
					...
					RootComp->SetCollisionResponseToChannel(ECollisionChannel::ECC_WorldStatic, ECR_Overlap);
				}
				break;
			}
			else
